...
let print_expr =
     Geometry.Symbolic.sexp_of_expr
...
let concrete_map variables widget =
  ...
  let rec collect map widget =
    let id = widget#id in

    Geometry.Map.add id (get_geometry id) map
    |> Widget.Map.fold (fun id widget map ->
         collect map widget
       ) widget#children
  in

  collect Geometry.Map.empty widget
...
module Solver (Screen : Size) = struct
  ...
  let rec expr_term parent siblings self expr =
    ...
    let binary a op b =
      perform with module Option.Monad in
        a <-- continue a;
        b <-- continue b;
        Some (op a b)
    in
  ...
  let rec collect_terms parent siblings id widget terms =
    let terms =
      Widget.Map.fold (collect_terms id (child_ids widget)) widget#children terms
    in
...
let solve screen widget =
  ...
  if debug then begin
    ...
    List.iter (fun cstr ->
      ...
      print_newline ();
    ) (Cstr.active_store () @ terms |> List.sort)
  end;
  ...
  let best = ref Geometry.Map.empty in
